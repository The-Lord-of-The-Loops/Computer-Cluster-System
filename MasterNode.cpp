...
bool MasterNode::complete(int ID, MachineType type)
{
	...
	case GP:
		...
		if (machine.getID() == ID)
		{
            machine.NoOfProcessesLeftTillMaintenance--;
		    if(machine.NoOfProcessesLeftTillMaintenance <= 0){
		        machine.MaintenanceTimeLeft = machine.MaintenanceTime;
		        InMaint_GP_Machines.enqueue(machine,machine.getResponseTime());
		    }else{
	...
	case GU:
		...
		if (machine.getID() == ID)
		{
            ...
            if(machine.NoOfProcessesLeftTillMaintenance <= 0){
                machine.MaintenanceTimeLeft = machine.MaintenanceTime;
                InMaint_GU_Machines.enqueue(machine,machine.getResponseTime());
            }else {
                if (machine.NoProcesses < N) {
	...
	case IO:
		...
		if (machine.getID() == ID)
		{
            ...
            if(machine.NoOfProcessesLeftTillMaintenance <= 0){
                machine.MaintenanceTimeLeft = machine.MaintenanceTime;
                InMaint_IO_Machines.enqueue(machine,machine.getResponseTime());
            }else {
                if (machine.NoProcesses < N) {
...
bool MasterNode::Assign(Process &process)
{
	...
	if (process.GetProcessType() == System)
	{
		...
		if(!InMaint_GP_Machines.isEmpty()){
            InMaint_GP_Machines.dequeue(mach);
            mach.inextime = clock + mach.getResponseTime() + mach.LateResponseDelayInCycles + process.GetExecutionTime();
            mach.NoProcesses++;
            EX_GP_Machines.enqueue(mach, mach.inextime);
            process.WT = process.WT + mach.getResponseTime();
            process.SetExecutionTime(process.GetExecutionTime() + mach.LateResponseDelayInCycles + mach.getResponseTime());
            process.Assigncycle = clock;
            process.AssignedLastCycle = true;
            process.AssignedMachineID = mach.getID();
            process.AssignedMachineType = mach.getMachineType();
        ...
        if(!InMaint_IO_Machines.isEmpty()){
            InMaint_IO_Machines.dequeue(mach);
            mach.inextime = clock + mach.getResponseTime() + mach.LateResponseDelayInCycles + process.GetExecutionTime();
            mach.NoProcesses++;
            EX_IO_Machines.enqueue(mach, mach.inextime);
            process.WT = process.WT + mach.getResponseTime();
        ...
        if(!InMaint_GU_Machines.isEmpty()){
            InMaint_GU_Machines.dequeue(mach);
            mach.inextime = clock + mach.getResponseTime() + mach.LateResponseDelayInCycles + process.GetExecutionTime();
            mach.NoProcesses++;
            EX_GU_Machines.enqueue(mach, mach.inextime);
            process.WT = process.WT + mach.getResponseTime();
...
void MasterNode::PrintInfo()
{
	//Available Machines
	PrintAvMacIDs();
	//In Maintenance Machines
	PrintInMaintenanceMachines();
	//Waiting Processes
	PrintWaProcIDs();
...
void MasterNode::CheckMaintenance() {
    LinkedQueue<Machine> Temp;
    Machine p ;
    while(InMaint_GP_Machines.dequeue(p)){
        p.MaintenanceTimeLeft--;
        if(p.MaintenanceTimeLeft <= 0)
        {
            p.NoOfProcessesLeftTillMaintenance = p.NoOfProcessesToMaintenance;
            AV_GP_Machines.enqueue(p,p.getResponseTime());
        }else{
            Temp.enqueue(p);
        }
    }
    while(Temp.dequeue(p)){
        InMaint_GP_Machines.enqueue(p,p.getResponseTime());
    }

    while(InMaint_GU_Machines.dequeue(p)){
        p.MaintenanceTimeLeft--;
        if(p.MaintenanceTimeLeft <= 0)
        {
            p.NoOfProcessesLeftTillMaintenance = p.NoOfProcessesToMaintenance;
            AV_GU_Machines.enqueue(p,p.getResponseTime());
        }else{
            Temp.enqueue(p);
        }
    }
    while(Temp.dequeue(p)){
        InMaint_GU_Machines.enqueue(p,p.getResponseTime());
    }

    while(InMaint_IO_Machines.dequeue(p)){
        p.MaintenanceTimeLeft--;
        if(p.MaintenanceTimeLeft <= 0)
        {
            p.NoOfProcessesLeftTillMaintenance = p.NoOfProcessesToMaintenance;
            AV_IO_Machines.enqueue(p,p.getResponseTime());
        }else{
            Temp.enqueue(p);
        }
    }
    while(Temp.dequeue(p)){
        InMaint_IO_Machines.enqueue(p,p.getResponseTime());
    }

}

void MasterNode::PrintInMaintenanceMachines() {
    Machine mach;
    PriorityQueue<Machine> gpque, guque, ioque;
    cout << "In Maintenance Machines: ";
    while (!InMaint_GP_Machines.isEmpty())
    {
        InMaint_GP_Machines.dequeue(mach);
        UI::PrintGPMachine(mach.getID());
        gpque.enqueue(mach, mach.getResponseTime());
    ...
    while (!InMaint_GU_Machines.isEmpty())
    {
        InMaint_GU_Machines.dequeue(mach);
        UI::PrintGPUMachine(mach.getID());
        guque.enqueue(mach, mach.getResponseTime());
    }
    while (!InMaint_IO_Machines.isEmpty())
    {
        InMaint_IO_Machines.dequeue(mach);
        UI::PrintIOMachine(mach.getID());
        ioque.enqueue(mach, mach.getResponseTime());
    ...
    {
        gpque.dequeue(mach);
        InMaint_GP_Machines.enqueue(mach, mach.getResponseTime());
    }
    while (!guque.isEmpty())
    {
        guque.dequeue(mach);
        InMaint_GU_Machines.enqueue(mach, mach.getResponseTime());
    }
    while (!ioque.isEmpty())
    {
        ioque.dequeue(mach);
        InMaint_IO_Machines.enqueue(mach, mach.getResponseTime());
    }
}
